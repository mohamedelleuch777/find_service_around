        ...
        <div style={{ display: 'grid', gap: '1.2rem' }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
              ...
              <span style={{ color: '#475569' }}>{jobs.length} total</span>
            </div>
            {jobs.length === 0 && <div style={{ color: '#475569' }}>No jobs yet. Hire a provider to start.</div>}
            {jobs.map((job) => {
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <div style={{ fontWeight: 700 }}>
                      {job.title || provider?.jobName || provider?.categoryName || 'Job'} • {job.status}
                    </div>
                    {job.status === 'in_progress' && (
                      <button
                        type="button"
                        onClick={() => setEndForm({ jobId: job.id, reason: 'completed', comment: '', rating: '' })}
                        style={{
                  ...
                  <div style={{ color: '#475569' }}>
                    With: {provider ? `${provider.firstName} ${provider.lastName}` : job.providerId}
                  </div>
                    ...
                    <div style={{ display: 'grid', gap: '0.5rem', marginTop: '0.35rem' }}>
                      ...
                      <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                        ...
                        <textarea
                          ...
                          onChange={(e) => setEndForm({ ...endForm, comment: e.target.value })}
                          rows={3}
                          style={{ padding: '0.65rem', borderRadius: 10, border: '1px solid #cbd5e1' }}
                        />
                      </label>
                      <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                        <span>Rate provider (0-5, one decimal) optional</span>
                        <input
                          type="number"
                          min={0}
                          max={5}
                          ...
                          onChange={(e) => setEndForm({ ...endForm, rating: e.target.value })}
                          style={{ padding: '0.65rem', borderRadius: 10, border: '1px solid #cbd5e1', width: '140px' }}
                        />
                      </label>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              const data = await res.json();
                              if (res.ok) {
                                setJobs(jobs.map((j) => (j.id === endForm.jobId ? { ...j, status: 'pending_provider', endRequest: data.job?.endRequest || null } : j)));
